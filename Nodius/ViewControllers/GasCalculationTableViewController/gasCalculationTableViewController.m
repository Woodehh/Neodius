...
- (void)viewDidLoad {
    ...
    UIImage *calculatorIcon = [[NodiusDataSource sharedData] tableIconPositive:@"fa-calculator"];
...
-(void)showWalletSelection {
    ...
    NSArray* storedWallets = [[NodiusDataSource sharedData] getStoredWallets];
...
-(void)calculateForWallet:(NSString*)walletAddress {
    ...
    [networkManager GET:[[NodiusDataSource sharedData] buildAPIUrlWithEndpoint:[NSString stringWithFormat:@"/address/balance/%@",walletAddress]]
             parameters:nil
               progress:nil
...
-(void)calculateWithAmount:(float)amount {
    ...
    [[NodiusDataSource sharedData] calculateBlockGenerationTimeWithCompletionBlock:^(CGFloat blockGenerationTime, NSError *error) {
        if (!error) {
        ...
        t_gas = [[NodiusDataSource sharedData] calculateGasForNeo:amount];
        
        //calculate actual gas
        a_gas    = [[NodiusDataSource sharedData] calculateGasForNeo:amount andBlockGenerationTime:[generationTime floatValue]];
...
-(UIView*)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    creditLabel.attributedText = [self setLineheightForString:NSLocalizedString(@"GAS Calculation has been brought to life by N1njaWTF of NeoToGas.com. Nodius' encourages you to tip some NEO/GAS to N1njaWTF for his awesome work!",nil)];
    creditLabel.frame = [[NodiusUIComponents sharedComponents] calculateAutoHeightForField:creditLabel andMaxWidth:creditLabel.frame.size.width];
...
-(void)showTipJar {
    [[NodiusUIComponents sharedComponents] showQrModalOnViewController:self
