...
-(void)viewWillAppear:(BOOL)animated {
    ...
    storedWallets = [[NodiusDataSource sharedData] getStoredWallets];
    UIComponents = [NodiusUIComponents sharedComponents];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            icon = @"fa-globe";
            cell.textLabel.text = [NSString stringWithFormat:NSLocalizedString(@"Visit %@",nil), @"Nodius.io"];
        } else if (indexPath.row == 1) {
    ...
    cell.imageView.highlightedImage = [[NodiusDataSource sharedData] tableIconNegative:icon];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
                                  ...
                                  WithCompletionBlock:^(bool addressEntered, NSString *walletName, NSString *walletAddress) {
                                      if (addressEntered) {
                                          if (![walletName isEqualToString:@""]) {         
                                              [[NodiusDataSource sharedData] addNewWallet:walletName withAddress:walletAddress];
                                              storedWallets = [[NodiusDataSource sharedData] getStoredWallets];
                                              [self.tableView reloadData];
                                          }
                                      }
        ...
        if (indexPath.row == 0) {
            ...
            baseMarketInfoViewController *menuItem = [[baseMarketInfoViewController alloc] init];
            menuItem.type = @"NEO";
            self.viewDeckController.centerViewController = [[UINavigationController alloc] initWithRootViewController:menuItem];
        } else if (indexPath.row == 3) {
            baseMarketInfoViewController *menuItem = [[baseMarketInfoViewController alloc] init];
            ...
            menuItem.closeButtonImage = [[NodiusDataSource sharedData] tableIconPositive:@"fa-reorder"];
            ...
            menuItem.leftBarButton = [[UIBarButtonItem alloc] initWithImage:[[NodiusDataSource sharedData] tableIconPositive:@"fa-reorder"]
        ...
        if (indexPath.row == 0) {
            ...
            [application openURL:[NSURL URLWithString:@"https://nodius.io"] options:@{} completionHandler:nil];
        } else if (indexPath.row == 1) {
            UIApplication *application = [UIApplication sharedApplication];
...
-(void)resetKeychain {
    ...
    [[NodiusDataSource sharedData] resetKeychain];
}
