...
- (void)viewDidLoad {
    [super viewDidLoad];
    storedWallets = [[NSMutableArray alloc] initWithArray:[[NodiusDataSource sharedData] getStoredWallets]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        cell.imageView.image = [[NodiusDataSource sharedData] tableIconPositive:@"fa-plus-circle"];
        cell.imageView.highlightedImage = [[NodiusDataSource sharedData] tableIconNegative:@"fa-plus-circle"];
    } else {
        cell.textLabel.text = storedWallets[indexPath.row][@"name"];
        cell.detailTextLabel.text = storedWallets[indexPath.row][@"address"];
        cell.imageView.image = [[NodiusDataSource sharedData] tableIconPositive:@"fa-folder-o"];
        cell.imageView.highlightedImage = [[NodiusDataSource sharedData] tableIconNegative:@"fa-folder-o"];
    ...
    if (indexPath.section == 0) {
                                         ...
                                         WithCompletionBlock:^(bool addressEntered, NSString *walletName, NSString *walletAddress) {
                                             if (addressEntered) {
                                                 if (![walletAddress isEqualToString:@""]) {
                                                     [[NodiusDataSource sharedData] addNewWallet:walletName
                                                                                      withAddress:walletAddress];
                                                     
                                                     //reload the stored wallets
                                                     storedWallets = (NSMutableArray*)[[NodiusDataSource sharedData] getStoredWallets];
        ...
        av.tapBlock = ^(UIAlertView *alertView, NSInteger buttonIndex) {
            if (buttonIndex == alertView.firstOtherButtonIndex) {
                ...
                [[NodiusDataSource sharedData] updateWalletAtIndex:indexPath.row
                 ...
                 otherButtonTitles:@[NSLocalizedString(@"Yes", nil)]
                          tapBlock:^(UIAlertView *alertView, NSInteger buttonIndex) {
                              if (buttonIndex == alertView.firstOtherButtonIndex) {
                                  ...
                                  [[NodiusDataSource sharedData] setStoredWallets:storedWallets];
                              }
                          }];
    }
}
